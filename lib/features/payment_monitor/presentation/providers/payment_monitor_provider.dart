...
part 'payment_monitor_audio_runtime.dart';
part 'payment_monitor_action_runtime.dart';
...
class PaymentMonitorProvider extends ChangeNotifier
  ...
  late final PaymentMonitorAudioRuntime _audioRuntime;
  late final PaymentMonitorSpeakerQueue _speakerQueue;
  late final PaymentMonitorActionRuntime _actionRuntime;
  @override
  ...
  late final PaymentMonitorRealtimeRuntime _realtimeRuntime;
  @override
  User? _user;
  @override
  ...
  PaymentMonitorProvider(
    ...
    _audioRuntime = PaymentMonitorAudioRuntime(
      repository: _repository,
      speaker: _speaker,
      amountAudioComposer: _amountAudioComposer,
      playbackRetryDelay: _playbackRetryDelay,
      voicePresetId: () => _speakerVoicePresetId,
      currentSpeakerGeneration: () => _speakerGeneration,
      isCurrentSpeakerOperation: _isCurrentSpeakerOperation,
      acknowledgeEvent: _acknowledgeNotificationEvent,
      onTerminal: _setTerminalNotification,
    );
    _realtimeRuntime = PaymentMonitorRealtimeRuntime(
      realtimeClient: _realtimeClient,
      onEnvelope: _handleRealtimeEnvelope,
      onSyncRequest: _handleRealtimeSyncRequest,
    ...
    _actionRuntime = PaymentMonitorActionRuntime(
      repository: _repository,
      findTransactionById: _findTransactionById,
      findTransactionByKey: _findTransactionByKey,
      replaceTransaction: _replaceTransaction,
      refreshCurrentPage: _refreshCurrentPageAfterOrderAction,
      showRowMessage: _showRowMessage,
      notifyListeners: notifyListeners,
      updatingOrderIds: _updatingOrderIds,
      updatingOrderTrackingIds: _updatingOrderTrackingIds,
    );
  ...
  bool get _isForeground => _realtimeRuntime.isForeground;
  bool get _isSpeakerBackgroundRuntimeAllowed =>
      _realtimeRuntime.isSpeakerBackgroundRuntimeAllowed;
  bool get _isListViewActive => _realtimeRuntime.isListViewActive;
  ...
  void syncRuntime({
    ...
    final runtimeChange = _realtimeRuntime.syncRuntime(
      isForeground: isForeground,
      isListViewActive: isListViewActive,
      allowBackgroundSpeakerRuntime: allowBackgroundSpeakerRuntime,
    );
    ...
    _reconcile();
    if (_isForeground && _realtimeRuntime.refreshPending && _isActive) {
      _scheduleRealtimeRefresh(drainReadyNotifications: false);
    }
    if (runtimeChange.becameListActive && _isListViewActive && _isActive) {
      unawaited(
        _poll(
          force: true,
          includeTotal: true,
  ...
  void _handleRealtimeLeaseChanged(String reason, bool acquired) {
    unawaited(
      AppLogger.instance.info(
        'PaymentMonitor',
        acquired
  ...
  void _reconcile({
    ...
    _realtimeRuntime.syncBackgroundLease(reason: 'eligibility_reconciled');
    if (!_isSpeakerRuntimeActive) {
      _stopSpeakerReadyFallback('speaker_runtime_paused');
      return;
    }
  ...
  void _stop({required String reason, bool clearError = true}) {
    _pollRequestToken += 1;
    _speakerGeneration += 1;
    final hadPendingRealtimeRefresh = _realtimeRuntime.refreshPending;
    _realtimeRuntime.releaseBackgroundLease('monitor_stopped_$reason');
    _realtimeRuntime.clearPendingRefresh();
  ...
  Future<void> _handleRealtimeEnvelope(RealtimeEnvelope envelope) async {
    ...
    if (isTransactionEvent && !_isForeground) {
      _realtimeRuntime.markRefreshPending();
      await AppLogger.instance.info(
        'PaymentMonitorRealtime',
  ...
  void _scheduleRealtimeRefresh({required bool drainReadyNotifications}) {
    _realtimeRuntime.scheduleRefresh(
      drainReadyNotifications: drainReadyNotifications,
    );
  }
  ...
  Future<bool> updateOrders(String transactionId, String rawInput) async {
    return _actionRuntime.updateOrders(transactionId, rawInput);
  }

  Future<bool> updateOrderTracking(String transactionId, String status) async {
    return _actionRuntime.updateOrderTracking(transactionId, status);
  }
  ...
  Future<bool> requestOrderTransfer(
    ...
    return _actionRuntime.requestOrderTransfer(transactionId, rawInput);
  }

  Future<List<BankStatementOrderHistoryEntry>> fetchOrderHistory(
    String transactionId,
  ) async {
    return _actionRuntime.fetchOrderHistory(transactionId);
  }
  ...
  Future<void> _reviewOrderTransferRequest(
    ...
    return _actionRuntime.reviewOrderTransferRequest(
      transactionId,
      requestId,
      approved: approved,
      note: note,
  ...
  void dispose() {
    _isDisposed = true;
    _realtimeRuntime.dispose(reason: 'provider_disposed');
    _speakerReadyFallbackTimer?.cancel();
    _clearRowMessages(notify: false);
    super.dispose();
  }
  ...
  Future<void> _playReadyNotifications(
    ...
    for (final notification in notifications) {
      ...
      try {
        ...
        final safeError = PaymentMonitorAudioRuntime.safeSpeakerError(error);
        _speakerError = PaymentSpeakerError(
          storeCode: notification.storeCode,
          notificationId: notification.notificationId,
          amount: notification.amount,
          occurredAt: DateTime.now(),
          message: _audioRuntime.buildSpeakerErrorMessage(safeError),
        );
        notifyListeners();
      } finally {
        if (!ownsActiveNotification) {
  ...
  Future<bool> _playNotificationWithRetry(
    ...
    return _audioRuntime.playNotificationWithRetry(
      notification,
      clientId,
      useStreamEndpoint: useStreamEndpoint,
      triggerSource: triggerSource,
