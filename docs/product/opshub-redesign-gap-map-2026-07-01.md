...
modal fullscreen. Ad-hoc `MaterialPageRoute` trong feature code cũng bị khóa,
chỉ cho phép scanner helper và image viewer BH/SC đã review.
...
- Web visual smoke automation 03/07/2026 đã thêm
  ...
  route bằng tokenless cached session (`/assignment-pending`) và 38
  ...
  Help Content admin, Phụ lục hợp đồng và Settings), tổng 84 route/viewport checks: không redirect sai route, không
  ...
  `ShellRoute` trong `AppRouter`, tổng 80 checks cho 2 viewport.
...
- Scanner follow-up 03/07/2026 sửa shared `BarcodeScannerScreen` để web/mobile
  ...
  (25 tests), `flutter analyze --no-pub`, full
  `flutter test --no-pub --reporter compact` (316 tests), và
...
- Shell route repaint proof 03/07/2026 đã pass sau khi phát hiện web, Windows
  ...
  `RepaintBoundary` theo route, đồng thời toàn bộ authenticated shell routes
  dùng `NoTransitionPage` để Navigator không giữ page cũ trong transition ngắn.
  BH/SC hub đổi về `AppResponsiveScrollView` và nút phụ trong header dùng kích
  thước theo nội dung để tránh ép text thành cột một ký tự. Validation đã pass
  focused warranty/router/Home regressions (19 tests), `flutter analyze --no-pub`,
  full `flutter test --no-pub --reporter compact` (299 tests),
  ...
  và `git diff --check` pass với cảnh báo CRLF trên Windows. Regression
  `test\app_shell_route_viewport_test.dart` khóa thêm việc đổi `AppShell`
...
- Route migration guard proof đã pass, xác nhận toàn bộ production UI không còn
  ...
  shell và scanner modal, đồng thời khóa ad-hoc `MaterialPageRoute` ngoài
  scanner modal/image viewer đã review.
  Validation sau slice guard đã pass `dart format`,
  focused `flutter test --no-pub --reporter expanded
  test\design_system_migration_guard_test.dart` (12 tests),
  `flutter analyze --no-pub`, full `flutter test --no-pub --reporter compact`
  (316 tests), và `git diff --check`.
