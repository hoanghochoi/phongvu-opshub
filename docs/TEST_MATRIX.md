...
| VIETQR-001 | Manual VietQR transfer QR creation screen, API payload generation with optional amount/content, persisted payment intent, and MAP transaction confirmation rule | yes | MAP live smoke partial | no | mobile smoke needed | changed | 2026-05-20: MAP UI search smoke succeeded; `npx prisma generate`, `npm run build`, `npm test -- --runInBand`, `flutter analyze`, `flutter test` |
