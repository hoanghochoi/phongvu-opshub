...
| HELP-001              | Public `/help` serves Markdown-authored staff guidance and roadmap content with image assets, `/download` links to it, the Flutter Home side menu opens the help page externally with `AppLogger` coverage, and Super Admin can manage a DB-backed runtime help mirror through `/admin/help-content` plus `/api/help-content/public` | yes     | help-content service/controller tests   | static route smoke           | Web, Android, Windows, Admin web              | changed             | 2026-07-04: added the `HelpContentPage` runtime table, `GET /api/help-content/public`, Super Admin-only `/api/admin/help-content/pages` create/update/restore endpoints, and the in-app `Quản lý hướng dẫn` markdown editor with restore-from-docs rollback. Asset upload is intentionally deferred; images still rely on `docs/help/assets/` and the static `/help` deploy path until the next cutover batch. Validation in this patch: `npx prisma validate`, `npx prisma generate`, backend `npm run build`, `npm test -- --runInBand src/help-content/help-content.service.spec.ts src/help-content/help-content.controller.spec.ts`, `node scripts/build-help-site.mjs`, `flutter analyze --no-pub`, `flutter test --no-pub --reporter expanded test/admin_menu_screen_test.dart`, and `git diff --check`. 2026-06-29: production workflow also listens to `help-content`; that branch is the production `docs/help` content source, pushes to it run only the static help/download deploy, and full `main` deploys load `docs/help` from `origin/help-content` when available. Gap: live `/help` smoke and runtime-editor smoke still need staging/prod verification after deploy; local Caddy validation remains unavailable because `caddy` is not installed. |
