...
| Atomic ingress/projection     | Valid/duplicate/conflict/mixed-invalid, control-lock race, lease/retry/dead-letter   | Partial: unit proof covers final-write control lock ordering, oversize, REQUESTID conflict, stable identity locks, under-lock conflict recheck, eligibility and expired-lease reclaim; real PostgreSQL race proof remains pending            |
| Migration                     | Fresh + upgraded scratch PostgreSQL, immutable audit, mode/legacy synchronization     | Partial: fail-closed `STOPPED` migration and synchronization trigger pass static proof; fresh/upgraded PostgreSQL and rollback-release rehearsal remain pending                                                                              |
| Flutter Admin                 | Three modes, readiness, backlog confirmation and four responsive breakpoints          | Blocked: no exact approved Figma revision/node map; production Flutter UI was intentionally not edited                                                                                                                                       |
...
| Environment-local KEK/deploy  | Bootstrap/reuse/legacy validation/backup/restore/blue-green/promotion boundary       | Partial: shell/static guards, candidate-before-install validation, age-only DB+KEK backup, restore verifier, shared mounts and runtime manifest proof pass; real deploy, mismatch injection and restore drill pending                         |
| Bank playbook                 | Local-only PDF render, reopen/text/secret scan and CI exclusion                       | Pass: ignored local PDF is three A4 pages; endpoint text and absence of internal KEK/mTLS/allowlist wording verified; tracked runbooks/generator removed and CI boundary guard passes                                                        |
