...
| PAYMENT-MONITOR-001 | Backend polls configured VietinBank MAP accounts, persists successful incoming transactions, QR confirmation checks persisted transactions first, and Windows PC speaks every newly observed amount in the background independent from OpsHub QR/payment intents | yes | MAP service test | no | Windows smoke needed | changed | 2026-05-21: added MAP transaction persistence tests, QR stored-match test, Flutter transaction parsing test, and Windows/web build proof; `npx prisma generate`, `npm run build`, `npm test -- --runInBand src/map-vietin/map-vietin.service.spec.ts src/vietqr/vietqr.service.spec.ts`, `flutter analyze`, `flutter test`, `flutter build windows --debug`, `flutter build web` |
