...
**Trạng thái rollout:** Các phase trong audit plan đã được triển khai và cập nhật proof đến 04/07/2026; các yêu cầu mới sau mốc này phải đi qua `docs/OPSHUB_MASTER_IMPLEMENTATION_PLAN.md`.
...
## 20. Execution Status
*   **Trạng thái thực thi:** 04 phase trong audit plan đã được triển khai qua các batch nhỏ từ 03/07/2026 đến 04/07/2026 và đều đã có proof tương ứng ngay trong từng phase ở mục 17.
*   **Kết quả đạt được:** IA được gọn lại theo AppShell hiện tại, màn Tasks dư thừa đã retire, header trùng lặp đã được dọn theo route active, các vấn đề quick wins và component polish đã được khóa bằng widget tests/guard tests.
*   **Biên giới tài liệu:** Audit plan này giờ đóng vai trò hồ sơ quyết định và checklist hồi quy cho rollout UI/UX 2026-07-03/04, không còn là backlog mở để tiếp tục thêm scope mới.
*   **Scope tiếp nối:** Các thay đổi mới ngoài phạm vi audit gốc như dashboard Home, `/operations`, Help CMS/runtime content, và canceled-order persistence phải tiếp tục ở `docs/OPSHUB_MASTER_IMPLEMENTATION_PLAN.md`.
*   **Close-out verification ngày 04/07/2026:** từ current repo state đã chạy lại `flutter analyze --no-pub`, full `flutter test --no-pub --reporter compact`, `flutter build web --debug --no-pub`, và `flutter build windows --debug --no-pub`; cả 4 lệnh đều pass. Đây là vòng proof toàn nền tảng cuối để chốt audit plan ở trạng thái completed/archive.

---

## 21. Risks & Mitigation
*   **Rủi ro tồn dư 1: Người dùng cũ có thể vẫn tìm màn `Tác vụ` cũ trong thói quen sử dụng.**
    *   *Giảm thiểu:* Giữ deep link `/tasks` chuyển về `/home`, giữ catalog tác vụ trên Home cho đến khi route `/operations` được chấp nhận và ship trong plan tiếp theo.
*   **Rủi ro tồn dư 2: Các màn mới tạo sau này có thể tái lộ heading trùng shell title hoặc copy nội bộ.**
    *   *Giảm thiểu:* Tiếp tục giữ `design_system_migration_guard_test.dart` và các test/grep guard liên quan như regression gate bắt buộc trước khi merge.
*   **Rủi ro tồn dư 3: Các polish animation/loading có thể regress trên platform hoặc viewport ít được chạm tới.**
    *   *Giảm thiểu:* Mỗi batch UI mới phải tiếp tục chạy `flutter analyze`, full `flutter test`, và build/smoke web + Windows trước khi chốt release.

---

## 22. Final Recommendation
Audit plan này có thể được xem là đã hoàn tất về mặt thiết kế và rollout kỹ thuật cho phạm vi gốc: quick wins, IA/navigation cleanup, header cleanup, component polish, accessibility guard, và responsive width lock đều đã có evidence trong repo.

Khuyến nghị sử dụng tài liệu từ mốc này:

1.  Giữ `docs/UI_UX_AUDIT_PLAN.md` như hồ sơ đóng scope và checklist hồi quy cho AppShell/Home/FIFO/BH-SC/finance/report/admin surfaces đã được dọn.
2.  Không tiếp tục nhồi scope mới vào audit plan này; mọi thay đổi UI/UX tiếp theo phải bám `docs/OPSHUB_MASTER_IMPLEMENTATION_PLAN.md` để tránh lẫn giữa phần audit đã chốt và backlog triển khai mới.
3.  Trước khi chốt release có chạm UI, tiếp tục dùng các proof đã chuẩn hóa trong repo: `flutter analyze`, full `flutter test`, và build/smoke các nền tảng đang ship được từ môi trường hiện tại.
