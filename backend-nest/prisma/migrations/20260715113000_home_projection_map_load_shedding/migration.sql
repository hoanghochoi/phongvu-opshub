-- MAP đồng bộ nhanh theo nhịp 1-2 giây. Dùng trailing debounce dài hơn cho
-- nguồn này để một burst giao dịch chỉ tạo một lần rebuild, nhưng vẫn chặn
-- starvation bằng ngưỡng tối đa 5 giây. Các nguồn còn lại giữ hợp đồng cũ.
...
DECLARE
    ...
    v_debounce INTERVAL := CASE
        WHEN p_source = 'MAP_VIETIN' THEN INTERVAL '2 seconds'
        ELSE INTERVAL '500 milliseconds'
    END;
    v_max_wait INTERVAL := CASE
        WHEN p_source = 'MAP_VIETIN' THEN INTERVAL '5 seconds'
...
BEGIN
    ...
    INSERT INTO "HomeSummaryProjectionQueue" (
        ...
        v_now, v_now + v_debounce, NULL, 0, NULL, v_now, v_now
    )
...
-- Provider thường trả lại cùng một giao dịch với rawData hoặc updatedAt mới.
-- Home chỉ phụ thuộc ngày, showroom, số tiền và danh sách đơn; bỏ qua UPDATE
-- không đổi các trường này để không tạo projection job giả.
...
BEGIN
    IF TG_OP = 'UPDATE' AND ROW(
        NEW."storeCode", NEW."amount", NEW."orders",
        NEW."paidAt", NEW."firstSeenAt"
    ) IS NOT DISTINCT FROM ROW(
        OLD."storeCode", OLD."amount", OLD."orders",
        OLD."paidAt", OLD."firstSeenAt"
    ) THEN
        RETURN NEW;
    END IF;
