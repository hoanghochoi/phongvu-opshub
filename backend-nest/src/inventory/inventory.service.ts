        ...
        try {
            ...
            const data = rows
                ...
                .map((row: any) => ({
                    ...
                    skuName: String(row.skuName || row.sku_name || row.SKU_NAME || '').trim(),
                    serialNumber: (row.serial_number || row.serialNumber || row.SERIAL_NUMBER)
                        ? String(row.serial_number || row.serialNumber || row.SERIAL_NUMBER).trim()
                        : null,
                    bin: (row.bin_id || row.bin || row.BIN)
                        ? String(row.bin_id || row.bin || row.BIN).trim()
                        : null,
                    zone: (row.zone || row.ZONE)
                        ? String(row.zone || row.ZONE).trim()
                        : null,
                    importDate: (row.import_date_company || row.import_date || row.importDate || row.IMPORT_DATE)
                        ? new Date(row.import_date_company || row.import_date || row.importDate || row.IMPORT_DATE)
                        : null,
                    count: (row.qty || row.count || row.COUNT)
