...
export class WarrantyController {
    ...
    // GET /warranties/search?receipt=xxxxx — mirrors bhsc-search
    @Get('search')
    async search(@Query('receipt') receipt: string) {
        return this.warrantyService.searchByReceipt(receipt || '');
    }

    // GET /warranties/detail?receipt=xxxxx — mirrors bhsc-get
    @Get('detail')
    ...
    async updateStatus(
        ...
        return this.warrantyService.updateWarrantyStatus(id, req.user.id, status as any);
    }
}
